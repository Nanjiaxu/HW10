...
function decide_response(user_said) {
  var response;
  var play_re = /call\s(.+)/i;  // creating a regular expression
  ...
  if (play_parse_array && state === "initial") {
    response = "ok, calling " + play_parse_array[1];
  } else if (user_said.toLowerCase().includes("calling") && state === "initial") {
    response = "call who?";
    state = "calling"
  } else if (user_said.toLowerCase().includes("hello")) {
    response = "hello, you can try to say calling";
    state = "initial"
  } else if (user_said.toLowerCase().includes("hi")) {
    response = "hello, you can try to say calling";
    state = "initial"
  } else if (state === "calling") {
    response = "ok, calling " + user_said;
    state = "initial"
  } else {
    response = "hello, you can try to say calling";
  }
